...
const copyFile = (sourceFile: string, targetFile: string, fileName: string) => {
...
const copyExistingFiles = async (addComponents: boolean) => {
  ...
  copyFile(
    path.join(sourceDir, "moon-core.css"),
    path.join(outputDir, "moon-core.css"),
    "moon-core.css"
  );

  if (addComponents) {
    copyFile(
      path.join(sourceDir, "moon-components.css"),
      path.join(outputDir, "moon-components.css"),
      "moon-components.css"
    );
  }
};

const generateFromFigma = async (addComponents: boolean) => {
  generateConfigFile();
  const config = getConfig();
...
const install = async () => {
  ...
  try {
    ...
    handleError(
      error,
      process.env.FIGMA_TOKEN ? "install script" : "copying CSS files"
    );
  }
};
